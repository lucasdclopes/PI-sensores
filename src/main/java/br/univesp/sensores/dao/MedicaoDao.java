	...
	public MedicaoListaResp listarAgrupado(final PaginacaoQueryParams paginacao, final DtParams dtParams, TipoAgrupamento tipoAgrupamento) {
		...
		String sql = """
			select 
				...
				,cast(avg(m.vlTemperatura) as decimal(6,2)),cast(avg(m.vlUmidade) as decimal(5,2)),max(m.dtMedicao) as dtMed
		...
		for (Object[] el : rs) {
			resultList.add(new MedicaoItemResp(
					(Long)el[0], (BigDecimal)el[1], (BigDecimal)el[2], ((Timestamp)el[3]).toLocalDateTime())
					);
		}
